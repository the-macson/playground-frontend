...
import { Card, Input, Button, Select, SelectItem, Textarea, Divider, Chip, Spinner } from '@nextui-org/react';
...
export default function EditProblem({ params }) {
  ...
  const fetchProblemData = async () => {
    try {
      ...
      setSelectedTags(new Set(data.tags.map((t) => t.id.toString())));
      setInputOutput(
        data.problemIOs.map((io) => ({
          ...io,
          id: io.id ? io.id.toString() : `tc-${Math.random().toString(36).substr(2, 9)}`,
        })),
      );
    } catch (error) {
  ...
  if (fetching) {
    return (
      <div className='min-h-screen bg-[#020617] flex items-center justify-center'>
        <Spinner size='lg' color='primary' label='Accessing Challenge Data...' className='font-mono text-sky-400' />
      </div>
    );
  }
        ...
        <div className='flex justify-between items-center'>
          ...
          <div className='text-right'>
            <h1 className='text-3xl font-black tracking-tighter text-white font-heading uppercase italic'>
              EDIT <span className='text-sky-400'>CHALLENGE</span>
            </h1>
            <p className='text-default-500 font-mono text-[9px] uppercase tracking-widest'>
              RANKING ID: #{id.padStart(4, '0')}
            </p>
          </div>
        </div>
